...
class TestInventoryReconciliation(unittest.TestCase):
	...
	def _test_reco_sle_gle(self, valuation_method):
		...
		for d in input_data:
			...
			stock_reco = create_inventory_reconciliation(qty=d[0], rate=d[1],
													 posting_date=d[2], posting_time=d[3])

			# check stock value
			sle = frappe.db.sql("""select * from `tabStock Ledger Entry`
				where voucher_type='Inventory Reconciliation' and voucher_no=%s""", stock_reco.name, as_dict=1)
...
def create_inventory_reconciliation(**kwargs):
	inventory_recon = _create_inventory_reconciliation(**kwargs)
	try:
		inventory_recon.submit()
	except EmptyStockReconciliationItemsError:
		pass
	return inventory_recon


def _create_inventory_reconciliation(**kwargs):
	args = frappe._dict(kwargs)
	inventory_recon = frappe.new_doc("Inventory Reconciliation")
	inventory_recon.posting_date = args.posting_date or nowdate()
	inventory_recon.posting_time = args.posting_time or nowtime()
	inventory_recon.set_posting_time = 1
	inventory_recon.company = args.company or "_Test Company"
	inventory_recon.expense_account = args.expense_account or \
									  ("Stock Adjustment - _TC" if frappe.get_all(
										  "Stock Ledger Entry") else "Temporary Opening - _TC")
	inventory_recon.cost_center = args.cost_center or "_Test Cost Center - _TC"
	inventory_recon.append("items", {
